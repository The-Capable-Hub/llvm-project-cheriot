...
QualType ASTContext::getVariableArrayType(QualType EltTy, Expr *NumElts,
  ...
  auto *New = new (*this, alignof(VariableArrayType))
      VariableArrayType(EltTy, Canon, NumElts, ASM, IndexTypeQuals, PIK);
  ...
  if (!canonTy) {
    canonTy = new (*this, alignof(DependentSizedArrayType))
        DependentSizedArrayType(QualType(canonElementType.Ty, 0), QualType(),
                                numElements, ASM, elementTypeQuals, PIK);
...
const RecordDecl *ASTContext::getCHERIClassDecl() const {
  if (!CHERIClassDecl) {
    RecordDecl *RD;
    ...
    case 'g': {
      ...
      if (const auto *PT = Type->getAs<PointerType>())
        ...
        assert(BT &&
               ...
               "Modifiers 'g' and 'l' only apply to pointers, references and "
...
QualType ASTContext::GetBuiltinType(unsigned Id,
  ...
  FunctionType::ExtInfo EI(IsCheriLibcall ? Target->getLibcallCallingConv()
