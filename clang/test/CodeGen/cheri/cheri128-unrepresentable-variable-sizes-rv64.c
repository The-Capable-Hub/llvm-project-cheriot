...
// RUN: %riscv64_cheri_purecap_clang -ffunction-sections -fdata-sections %s -S -o - -emit-llvm | FileCheck %s -check-prefix IR
// FIXME: update test for -fno-common
// RUN: %riscv64_cheri_purecap_clang -ffunction-sections -fdata-sections %s -S -o %t.s -fcommon
// RUN: FileCheck %s -check-prefix ASM -input-file=%t.s
// RUN: %riscv64_cheri_purecap_clang -ffunction-sections -fdata-sections %s -c -o - -fcommon | llvm-readelf --sections --symbols - | FileCheck %s -check-prefix SECTIONS
// Check that we can compile the generated assembly and it matches the direct emission
// RUN: %riscv64_cheri_purecap_clang %t.s -c -o - | llvm-readelf --sections --symbols - | FileCheck %s -check-prefix SECTIONS
...
// SECTIONS-NEXT: NULL 0000000000000000 {{[0-9a-f]+}} 000000 00 0 0 0
// SECTIONS-NEXT: .strtab STRTAB 0000000000000000
// SECTIONS-NEXT: .text PROGBITS 0000000000000000 {{[0-9a-f]+}} 000000 00 AX 0 0 2
...
// SECTIONS: 0000000000000001 128 OBJECT GLOBAL DEFAULT COM common128
// SECTIONS: 0000000000000008 4096 OBJECT GLOBAL DEFAULT COM common4096
// SECTIONS: 0000000000000080 65535 OBJECT GLOBAL DEFAULT COM common65535
// SECTIONS: 0000000000000080 65537 OBJECT GLOBAL HIDDEN COM common65537
// SECTIONS: 0000000000000100 139267 OBJECT GLOBAL PROTECTED COM common139267
...
// ASM-NEXT: .byte 4
// ASM-NEXT: .zero 65536
// ASM-NEXT: .zero 127 # Tail padding to ensure precise bounds
...
// ASM-NEXT: .p2align 8, 0x0
// ASM-NEXT: char139267:
// ASM-NEXT: .byte 5
// ASM-NEXT: .zero 139266
// ASM-NEXT: .zero 253 # Tail padding to ensure precise bounds
