...
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+xcheripurecap,+f %s -o - -O2 -verify-machineinstrs | FileCheck %s
...
; CHECK-NEXT:    csc s0, 0(sp) # 8-byte Folded Spill
...
; CHECK-NEXT:    clc a0, 8(s0)
; CHECK-NEXT:    csc a0, 8(a1)
; CHECK-NEXT:    clc a0, 16(s0)
; CHECK-NEXT:    csc a0, 16(a1)
; CHECK-NEXT:    clc a2, 24(s0)
; CHECK-NEXT:    li a0, 1
; CHECK-NEXT:    csc a2, 24(a1)
; CHECK-NEXT:    clc ra, 8(sp) # 8-byte Folded Reload
; CHECK-NEXT:    clc s0, 0(sp) # 8-byte Folded Reload
; CHECK-NEXT:    cincoffset sp, sp, 16
; CHECK-NEXT:    cret
entry:
