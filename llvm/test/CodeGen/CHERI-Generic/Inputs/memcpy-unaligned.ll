...
; RUN: opt @PURECAP_HARDFLOAT_ARGS@ -passes=sroa,instsimplify %s -o - -S | FileCheck %s -check-prefix SROA
; RUN: opt @PURECAP_HARDFLOAT_ARGS@ -passes=sroa,instsimplify %s -o - -S | llc @PURECAP_HARDFLOAT_ARGS@ -O2 -verify-machineinstrs - -o - 2>%t.dbg | FileCheck %s
; RUN: FileCheck %s -check-prefix=DBG -input-file=%t.dbg
; DBG:      warning: <unknown>:0:0: in function spgFormLeafTuple void (): found underaligned store of capability type (aligned to 1 bytes instead of @CAP_BYTES@). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
; DBG-NEXT: warning: <unknown>:0:0: in function align2_should_call_memcpy void (): found underaligned store of capability type (aligned to 2 bytes instead of @CAP_BYTES@). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
; DBG-NEXT: warning: <unknown>:0:0: in function align4_should_call_memcpy void (): found underaligned store of capability type (aligned to 4 bytes instead of @CAP_BYTES@). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
@IFNOT-RISCV32@; DBG-NEXT: warning: <unknown>:0:0: in function align8_should_call_memcpy void (): found underaligned store of capability type (aligned to 8 bytes instead of @CAP_BYTES@). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
; DBG-EMPTY:
...
@IF-MIPS@  ; CHECK: clcbi $c12, %capcall20(memcpy)($c1)
@IF-RISCV64@  ; CHECK: ccall memcpy
@IF-RISCV32@  ; CHECK-NOT: memcpy
@IF-RISCV32@  ; CHECK: csc {{.+}}, 8(
@IF-RISCV32@  ; CHECK: csc {{.+}}, 0(
}
...
@IF-MIPS@  ; CHECK: clcbi $c12, %capcall20(memcpy)($c
@IFNOT-MIPS@  ; CHECK: ccall memcpy
}
...
@IF-MIPS@  ; CHECK: clcbi $c12, %capcall20(memcpy)($c
@IF-RISCV64@  ; CHECK: ccall memcpy
@IF-RISCV32@  ; CHECK-NOT: ccall memcpy
@IF-RISCV32@  ; CHECK: cincoffset a1, zero, 1
@IF-RISCV32@  ; CHECK-NEXT: csc a1, 0(a0)
}
...
@IF-MIPS@; CHECK-NOT: %capcall20(memcpy)
@IF-MIPS@; CHECK: clcbi $c1, %captab20(b)
; $c1 <- global void*
@IF-MIPS@; CHECK: clc $c1, $zero, 0($c1)
@IF-MIPS@; CHECK: cincoffset $c2, $cnull, 1
@IF-MIPS@; CHECK: csc $c2, $zero, 0($c1)
@IFNOT-MIPS@; CHECK-NOT: memcpy
@IFNOT-MIPS@; CHECK: clc a0, 0(a0)
@IFNOT-MIPS@; CHECK: cincoffset a1, zero, 1
@IFNOT-MIPS@; CHECK-NEXT: csc a1, 0(a0)
}
