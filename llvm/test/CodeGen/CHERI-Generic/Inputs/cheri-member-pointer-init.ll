; !DO NOT AUTOGEN!
; RUN: llc @PURECAP_HARDFLOAT_ARGS@ -cheri-cap-table-abi=pcrel %s -o - | FileCheck %s

; CHECK-LABEL: global_nonvirt_ptr:
; CHECK-NEXT:    .chericap _ZN1A7nonvirtEv
@IF-MIPS@; CHECK-NEXT:    .8byte 0
@IF-MIPS@; CHECK-NEXT:    .space 8
@IFNOT-MIPS@; CHECK-NEXT:    .quad 0
@IF-RISCV64@; CHECK-NEXT:    .zero 8
; CHECK-NEXT:    .size global_nonvirt_ptr, {{32|16}}

; CHECK-LABEL: global_virt_ptr:
; CHECK-NEXT:    .chericap 0
@IF-MIPS@; CHECK-NEXT:    .8byte 1
@IF-MIPS@; CHECK-NEXT:    .space 8
@IFNOT-MIPS@; CHECK-NEXT:    .quad 1
@IF-RISCV64@; CHECK-NEXT:    .zero 8
; CHECK-NEXT:    .size global_virt_ptr, {{32|16}}

; CHECK-LABEL: global_fn_ptr:
; CHECK-NEXT:    .chericap _Z9global_fnv
; CHECK-NEXT:    .size global_fn_ptr, @CAP_BYTES@
