...
; RUN: llc -mtriple=mips64 -mcpu=cheri128 -mattr=+cheri128 --relocation-model=pic -target-abi purecap -relocation-model=static %s -o - | FileCheck %s --check-prefix=ASM
...
; RUN:   FileCheck %s --check-prefix=RELOCS '-DCAP_RELOC=R_MIPS_CHERI_CAPABILITY/R_MIPS_NONE/R_MIPS_NONE'
