...
; RUN: opt -mtriple=mips64 -mcpu=cheri128 -mattr=+cheri128 --relocation-model=pic -target-abi purecap -passes=sroa,instsimplify %s -o - -S | FileCheck %s -check-prefix SROA
; RUN: opt -mtriple=mips64 -mcpu=cheri128 -mattr=+cheri128 --relocation-model=pic -target-abi purecap -passes=sroa,instsimplify %s -o - -S | llc -mtriple=mips64 -mcpu=cheri128 -mattr=+cheri128 --relocation-model=pic -target-abi purecap -O2 -verify-machineinstrs - -o - 2>%t.dbg | FileCheck %s
