...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -asm-verbose -verify-regalloc -O0 %s -o - | FileCheck %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -asm-verbose -verify-regalloc -O1 %s -o - | FileCheck %s --check-prefix=OPT
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -asm-verbose -verify-regalloc -O2 %s -o - | FileCheck %s --check-prefix=OPT
...
; CHECK-NEXT:    csc a1, 16(sp) # 16-byte Folded Spill
; CHECK-NEXT:    srai a1, a2, 1
; CHECK-NEXT:    cincoffset a0, a0, a1
; CHECK-NEXT:    csc a0, 32(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc a0, 48(sp) # 16-byte Folded Spill
; CHECK-NEXT:    andi a0, a2, 1
; CHECK-NEXT:    beqz a0, .LBB0_2
; CHECK-NEXT:    j .LBB0_1
; CHECK-NEXT:  .LBB0_1: # %memptr.virtual
; CHECK-NEXT:    clc a1, 16(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc a0, 48(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc a0, 0(a0)
; CHECK-NEXT:    mv a1, a1
; CHECK-NEXT:    cincoffset a0, a0, a1
; CHECK-NEXT:    clc t1, 0(a0)
; CHECK-NEXT:    csc t1, 0(sp) # 16-byte Folded Spill
; CHECK-NEXT:    j .LBB0_3
; CHECK-NEXT:  .LBB0_2: # %memptr.nonvirtual
; CHECK-NEXT:    clc t1, 16(sp) # 16-byte Folded Reload
; CHECK-NEXT:    csc t1, 0(sp) # 16-byte Folded Spill
; CHECK-NEXT:    j .LBB0_3
; CHECK-NEXT:  .LBB0_3: # %memptr.end
; CHECK-NEXT:    clc a0, 32(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc t1, 0(sp) # 16-byte Folded Reload
; CHECK-NEXT:    cincoffset sp, sp, 64
; CHECK-NEXT:    cjr t1
;
