...
; RUN: opt -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -passes=sroa,instsimplify %s -o - -S | FileCheck %s -check-prefix SROA
; RUN: opt -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -passes=sroa,instsimplify %s -o - -S | llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -O2 -verify-machineinstrs - -o - 2>%t.dbg | FileCheck %s
